...
export default function AdminNeedyReport() {
	// Inputs
	...
	const fetchReport = async () => {
		if (!name && !year) {
			setError("Please provide at least a needy name and year.");
			return;
		}
		setReport(null);
		setLoading(true);
		...
		try {
			...
			const response = await fetch("http://localhost:3001/api/needy_report", {
				method: "POST",
				credentials: "include",
				...
				>
					<thead>
						<tr>
							...
							<th className="p-5">Total Value Received</th>
						</tr>
					</thead>
					<tbody>
						{report.map((plate) => (
							<tr key={plate.plate_id}>
								...
								<td>{plate.price}</td>
								<td>{plate.total_received}</td>
								<td>{plate.total_value_received}</td>
							</tr>
						))}
					</tbody>
				</table>
