...
export default function RestaurantPage() {
	...
	const fetchItems = async () => {
		try {
			const res = await fetch(
				`http://localhost:3001/api/restaurant/items?username=${username}`
			);
	...
	const handleDelete = async (pid) => {
		...
		try {
			const res = await fetch(
				`http://localhost:3001/api/restaurant/items/${pid}?username=${username}`,
				{ method: "DELETE" }
			);
			const data = await res.json();
			if (!res.ok) {
	...
	return (
		<div className="bg register-container">
			...
			<button className="login-button" onClick={() => setShowAddPopup(true)}>
				Add New Item
			</button>
					...
					<div className="popup-card" onClick={(e) => e.stopPropagation()}>
						{!editing ? (
							<>
								...
								<p>
									Window: {new Date(selectedItem.start_time).toLocaleString()} –{" "}
								...
								<button
									className="login-button"
									onClick={() => handleDelete(selectedItem.pid)}
								>
									Delete
								</button>
								<button
							...
							<>
								...
								<input
									type="datetime-local"
									value={new Date(selectedItem.start_time)
										.toISOString()
										.slice(0, 16)}
									onChange={(e) =>
										setSelectedItem({
								...
								<input
									type="datetime-local"
									value={new Date(selectedItem.end_time)
										.toISOString()
										.slice(0, 16)}
									onChange={(e) =>
										setSelectedItem({
